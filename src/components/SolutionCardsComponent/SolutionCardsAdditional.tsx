...
import { ICardForHamper } from './SliderSolutionsCards/data';
import SliderCards from './SliderCards';
import { cardsDataHamper, cardsDataRealEstate, cardsDataRestaurant, cardsDataSpeciallyMade } from './SliderCards/data';

interface ICardProps{
  array?:ICardForHamper[]
}

const SolutionCardsAdditional:FC<ICardProps> = ({array}) => {
  const { pathname } = useLocation();
  return (
   <>
    {pathname === "/real-estate-services" ? (
         <div className={styles.cardsContainer}>
           {cardsDataRealEstate.map(item=>(
            <div className={`${styles.cardContainerEstate}`}  >
          ...
          {pathname !== "/restaurant-services"  && pathname !== "/hamper" && pathname !== "/specially-made" && <>
                          ...
                          In the fast-paced world of real estate, it's essential to ensure potential buyers and renters see your listings the moment they're available. With IDF's cutting-edge technology, you're one step ahead, always.
                            ...
                            <ul>
                                ...
                                <li>
                                    ...
                                    <p className={styles.listText}>The moment you enter a new listing or make changes to an existing one, IDF ensures it's instantly reflected across every digital solution in our extensive network. No more manual updates. No more missed opportunities.</p>
                                </li>
                                  <li>
      ...
      {pathname === "/restaurant-services" && (
        <>
         <div className={styles.cardsContainerRestaurant}>
          {cardsDataRestaurant.map(item=>(
              <div className={`${styles.cardContainerRestaurant}`}  >
                <div className={styles.bodyRestaurant}>
                    <p className={styles.cardTitle}>{item.title}</p>
                    <h6 className={styles.cardHeader}>{item.header}</h6>
                    <p className={styles.cardDescriptionRestaurant}>{item.description}
              </p>
              <p className={styles.whyMatter}>Why It Matters</p>
              <p className={styles.cardDescriptionRestaurant}>{item.description2}</p>
              </div>
          </div>
           
          ))}
          </div>
          <SliderCards array={cardsDataRestaurant}/>
       ...
       {pathname === "/hamper" && (
        <>
        <div className={styles.cardsContainerHamper}>
          {cardsDataHamper.map(item =>(
            <div className={`${styles.cardContainerHamper}`}  >
                <div className={styles.body}>
                    <p className={styles.cardTitle}>{item.title}</p>
                    <h6 className={styles.cardHeader}>{item.header}</h6>
                    <p className={styles.cardDescription}>{item.description}</p>
      ...
      {pathname === "/specially-made" && (
        ...
        <SliderCards array={cardsDataSpeciallyMade}/>
        </>
      )}
      </>
  )
...
export default SolutionCardsAdditional;